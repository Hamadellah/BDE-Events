...
<head>
    ...
    <title>Événements Dashboard</title>
...
<body class="bg-[#F8FAFC] text-slate-800 min-h-screen font-sans antialiased relative overflow-x-hidden">
    ...
    <div class="max-w-7xl mx-auto p-4 md:p-8 flex flex-col lg:flex-row gap-8">
            ...
            <div class="sticky top-8 bg-white/80 backdrop-blur-xl border border-white p-6 rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] flex flex-col items-center text-center">
                ...
                <div class="w-full space-y-3 pt-4 border-t border-slate-100 text-left text-xs font-medium text-slate-500 mb-6">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-50/80 border border-slate-100">
                        <span class="text-slate-400">Événements Disponibles</span>
                        <span class="font-extrabold text-slate-800 text-sm">{{ count($event) }}</span>
                    </div>
                </div>
        ...
        <main class="flex-1 space-y-6">
            ...
            <!-- Top Header with "Mes Billets" Button -->
            <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-3xl bg-white/70 backdrop-blur-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Découvrir les Événements</h1>
                    <p class="text-xs text-slate-400 font-medium mt-0.5">Explorez et rejoignez les prochains événements</p>
                </div>

                <!-- Button "Mes Billets" -->
                <a href="{{ route('my-tickets') }}" method="get"
                   class="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-bold text-xs border border-indigo-100 transition-all duration-300 shadow-sm hover:shadow hover:-translate-y-0.5 shrink-0">
            ...
            @if(session('message'))
                <div class="flex items-center gap-3 p-4 rounded-2xl bg-emerald-50/80 backdrop-blur-md border border-emerald-200/60 text-emerald-800 text-sm font-semibold shadow-sm animate-fade-in">
                    ...
                    <article class="group relative rounded-[2.5rem] bg-white/80 backdrop-blur-xl border border-white p-7 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_-10px_rgba(79,70,229,0.12)] hover:border-indigo-100 flex flex-col justify-between overflow-hidden">
                        ...
                        <div>
                            ...
                            <div class="flex justify-between items-start mb-3 gap-3">
                                ...
                                <span class="bg-indigo-50 text-indigo-700 text-xs font-black px-3 py-1.5 rounded-xl border border-indigo-100/80 whitespace-nowrap shadow-sm">
                                    {{ $event->price }} DH
                                </span>
                            </div>
                            ...
                            <!-- Details Badge Cards -->
                            <div class="space-y-2 text-xs text-slate-600 mb-6 bg-slate-50/60 p-4 rounded-2xl border border-slate-100/80">
                                <!-- Date -->
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <span class="text-slate-500 font-medium">{{ $event->event_date }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <span class="text-slate-500 font-medium">{{ $event->start_time }} - {{ $event->end_time }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <span class="text-slate-500 font-medium truncate">{{ $event->location }}</span>
                                </div>

                                <!-- Capacity -->
                                <div class="flex items-center gap-2.5">
                                    <span class="p-1 rounded-lg bg-white border border-slate-100 shadow-2xs">👥</span>
                                    <span class="font-bold text-slate-700">Capacité:</span>
                                    <span class="text-slate-500 font-medium">{{ $event->capacity }} places</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('register-for-event', $event->id) }}" method="POST">
                            @csrf
                            <button type="submit" 
                                class="w-full group/btn relative inline-flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm transition-all duration-300 shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 active:scale-[0.98]">
                                <span>Intégrer à l'événement</span>
